...
class DistMult(torch.nn.Module):
    ...
    def emb_def(self):
        ...
        # )
        self.reduce=torch.nn.Conv2d(1, 1, 1, 1, 0)
        self.span = torch.nn.Conv2d(1, 9, 1, 1, 0)
        self.unfold=torch.nn.Unfold(3,1,1,1)
 
        self.bn2 = torch.nn.BatchNorm1d(self.embedding_size)
        self.fc = torch.nn.Linear(200,self.embedding_size)
    ...
    def form_query(self, e1, rel):
        ...
        e1_embedded= ent_emb.view(-1, 1, 10, 10)
        rel_embedded = rel_emb.view(-1, 1, 10, 10)
        stacked_inputs = torch.cat([e1_embedded, rel_embedded], 2)
        x= self.inp_drop(stacked_inputs)
        x_unfolded =self.unfold(x)
        x_unfolded = x_unfolded.view(batch_size, 1, 1, 3*3, 20, 10)
        kernel = self.span(self.reduce(x))
        kernel = kernel.view(batch_size, 1, 3*3, 20, 10).unsqueeze(2)
    ...
    def forward_triple(self, heads, rels, tails):
        ...
        e1_embedded= head_emb.view(-1, 1, 10, 10)
        rel_embedded = rel_emb.view(-1, 1, 10, 10)
        stacked_inputs = torch.cat([e1_embedded, rel_embedded], 2)
        x= self.inp_drop(stacked_inputs)
        x_unfolded =self.unfold(x)
